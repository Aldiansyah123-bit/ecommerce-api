...
class CartController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return new CartResource(true, "Success to Cart", $item);
    }
    ...
    public function getPrice()
    {
        $totalPrice = Cart::with('product')
                ->where('customer_id', auth()->guard('api_customer')->user()->id)
                ->sum('price');
        return new CartResource(true, 'Total Cart', $totalPrice);
    }
    ...
    public function getWeight()
    {
        $totalWeight = Cart::with('product')
                    ->where('customer_id', auth()->guard('api_customer')->user()->id)
                    ->sum('weight');
        return new CartResource(true, 'Total Weight', $totalWeight);
    }

    public function removeCart(Request $request)
    {
        $cart = Cart::with('pruduct')
                ->whereId($request->id)
                ->first();
        return new CartResource(true, 'Success remove Cart', null);
    }
}
